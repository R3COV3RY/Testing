...
for /f "usebackq delims=" %%S in ("%SERVERS%") do (
  ...
  for %%P in (%PORTS%) do (
    ...
    REM small throttle to reduce load
    timeout /t 1 >nul
  )
...
REM Try HTTP first, then HTTPS if HTTP not matched
call :FETCH_AND_MATCH "http"  "!H!" "!P!" "!PATH!"
if "!MATCHED!"=="1" goto :eof
...
REM Require >=2 indicators to reduce false positives
set /a HIT=0
...
if !HIT! GEQ 2 (
  ...
  echo [+] warning Web Shell found on %H%:%P%PATH%>> "%OUT%"
)
